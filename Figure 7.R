...
treeval.plot(temp, inferenceType=1, digits=2, printn=FALSE, nn=FALSE)
ggplot(data=covariates, aes(x=hunger,y=kcal24h0)) + geom_point() + geom_smooth()+xlab("Hunger")+ylab("24 Hour Calorie Intake")+
  theme_bw()+theme(axis.title.y = element_text(size = 7), axis.title.x = element_text(size = 8))

                        ...
                        minbucket=1,maxdepth=5, XORlev=1)
{
  ...
  while (j < length(splitList)) {
      ...
      write(paste(c(beta,XORlev, cp, seed, "Tree-Values", "split", depth, 
                    p_split, p_split_cons, p_split_SSE, p_split_mad, p_split_med_yc,
                    CI_split, CI_split_cons, CI_split_SSE, CI_split_mad ,CI_split_med_yc,
      ...
      write(paste(c(beta,XORlev, cp, seed, "Tree-Values", "child", depth, 
                    p_1, p_1_cons, p_1_SSE, p_1_mad, p_1_med_yc,
                    CI_1, CI_1_cons, CI_1_SSE, CI_1_mad, CI_1_med_yc,
                    length(y1), NA, sample_signal_1,true_signal_1,NA,
                    sum(size(phi_bounds_1)), sum(size(numerator_1))), collapse=" "),
            file=filename,append=TRUE)
      write(paste(c(beta,XORlev, cp, seed, "Tree-Values", "child", depth, 
                    p_2, p_2_cons, p_2_SSE, p_2_mad, p_2_med_yc,
                    CI_2, CI_2_cons, CI_2_SSE, CI_2_mad, CI_2_med_yc,

...
filename<- "normal_var_est_p30.csv"
...
for (i in 1:nTrials) {
  ...
  base_tree <- rpart::rpart(y~., data=dat,
                            control=rpart.control(minbucket=1,cp=0, maxcompete=0,maxsurrogate=0, maxdepth=3), model=TRUE)
  ...
  split_tree <- rpart::rpart(y~., data=dat1, control=rpart.control(minbucket=1,cp=0, maxcompete=0,maxsurrogate=0, maxdepth=3), model=TRUE)
...
#   #y <- rnorm(n, 0, sigma_y)
#   #y <- rpois(n,10)
#   y <- (1-2*rbinom(n, size=1, 0.5))*rnorm(n, mean=5, sd=1)
#   sigma_y <- sd(y)
#   dat <- data.frame(y=y,X=X)
#   nameX <- sapply(1:p, function(u) paste0("X",u))
#   names(dat) = c("y", nameX)

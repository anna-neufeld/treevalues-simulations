...
p4log <- ggplot(data=groupRes %>% filter(method=="Tree-Values",type=="child"), aes(x=beta, y=length, col=method2, lty=depth2))+ geom_smooth()+facet_grid(cols=vars(XORlev2))+ylab("Median Confidence Interval Width")+xlab("b")+
  theme_bw()+
...
ggsave(filename="~/Dropbox/Tree Values Paper/newformat_v35/Figures/CI_width_nolog_nolev.png")
